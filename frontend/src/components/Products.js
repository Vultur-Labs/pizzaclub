  ...
  render() {
    ...
    return (
      <div className="product-type" data-id={id}>
        ...
        <div className={show ? "product-shower" : "inactive"}>{children}</div>
      </div>
    );
  }
}
  ...
  return (
    <div className="quantity-widget">
      ...
      <div className="buttons has-addons quantity-widget-counter">
        <span
          className="button is-success is-medium is-outlined quantity-widget-btn-left"
          onClick={decrement}
        >
        ...
        <input
          className="input is-success is-medium quantity-widget-input"
          type="text"
          readOnly
          value={quantity}
          name={name}
        />
        <span
          className="button is-success is-medium is-outlined quantity-widget-btn-rigth"
          onClick={increment}
        >
          <span className="icon is-small">
...
const ProductOnlyPriceView = (props) => {
  const { image, name, description, prices } = props.data;
  ...
  _genItem = (item_id) => {
    ...
    let price = data.prices.filter(
      (i) =>
        ...
        Number(i.presentation) === Number(presentation)
    )[0];
